    ...
    def validate(self, epoch, save=False, visualize=False):
        ...
        with torch.no_grad():
            for i, (samples, class_target, box_target) in enumerate(self.trainloader):
                ...
                pred_bboxes = Transform_coor(out_bbox, box_target, class_target, nms_threshold=0.1, plot=False)

                # train only for classification for now
                loss = self.bbox_loss(box_target.to(device), class_target.to(device), out_bbox)

                ats_bounding_boxes = compute_ats_bounding_boxes(pred_bboxes, box_target)
                threat_scores.append(ats_bounding_boxes.item())
        ...
        if save:
            torch.save(self.model.state_dict(), 'test_bbox.pth')
